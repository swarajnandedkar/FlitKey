...
class ConfigSecurityTests(unittest.TestCase):
    def test_load_state_ignores_malformed_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {"XDG_CONFIG_HOME": tmpdir}, clear=False):
                target = Path(tmpdir) / "typeflow" / "config.json"
    ...
    def test_save_state_preserves_untrusted_text_without_execution(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {"XDG_CONFIG_HOME": tmpdir}, clear=False):
                ...
                saved = (Path(tmpdir) / "typeflow" / "config.json").read_text(encoding="utf-8")
    ...
    def test_legacy_config_migrates_to_typeflow_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {"XDG_CONFIG_HOME": tmpdir}, clear=False):
                ...
                migrated = Path(tmpdir) / "typeflow" / "config.json"
